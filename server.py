...
detector = None
detector_lock = __import__('threading').Lock()
...
def get_microplastic(mp_id):
    ...
    mp = Microplastic.query.get_or_404(mp_id)
...
def get_statistics():
    ...
    total = Microplastic.query.count()
    
    structure_dist = db.session.query(
        Microplastic.structure_type, func.count(Microplastic.id)
    ).group_by(Microplastic.structure_type).all()
    
    shape_dist = db.session.query(
        Microplastic.shape, func.count(Microplastic.id)
    ).group_by(Microplastic.shape).all()
    
    polymer_dist = db.session.query(
        Microplastic.polymer_type, func.count(Microplastic.id)
    ).group_by(Microplastic.polymer_type).all()
    
    risk_dist = db.session.query(
        Microplastic.risk_level, func.count(Microplastic.id)
    ).group_by(Microplastic.risk_level).all()
    
    sample_type_dist = db.session.query(
        Microplastic.sample_type, func.count(Microplastic.id)
    ).group_by(Microplastic.sample_type).all()
    
    avg_size = db.session.query(func.avg(Microplastic.length)).scalar() or 0
    avg_concentration = db.session.query(func.avg(Microplastic.concentration)).scalar() or 0
    avg_confidence = db.session.query(func.avg(Microplastic.confidence_score)).scalar() or 0
...
def get_reports():
    ...
    reports = AnalysisReport.query.order_by(AnalysisReport.created_date.desc()).all()
...
def export_data():
    ...
    microplastics = Microplastic.query.all()
